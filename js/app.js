...
<div class="wrap">
  ...
  async function boot(){
    ...
    // ====== FIX: Agar input file bisa pilih GALERI (bukan kamera saja) di mobile ======
    (function ensureGalleryPickers(){
      ...
      const patch = (el)=>{
        ...
        // Pastikan tipe file input benar
        try{
          if(String(el.type).toLowerCase() !== "file"){
            // kalau HTML-nya salah (mis. type bukan file), jangan crash
            return;
          }
        }catch(_){}

        // Hapus "capture" supaya tidak memaksa kamera
        // (di Android/Chrome, presence capture sering bikin langsung kamera)
        el.removeAttribute("capture");
        // beberapa template pakai capture="environment" / "user"
        try{ el.capture = ""; }catch(_){}

        // Pastikan accept mengizinkan gallery picker
        // Kalau Anda hanya butuh foto: image/*
        // Kalau butuh foto+video: "image/*,video/*"
        el.setAttribute("accept", "image/*");

        // Opsional: pastikan single file
        el.removeAttribute("multiple");

        // Android kadang “nyangkut” kalau label/button memicu click berulang
        // Ini bantu supaya open picker konsisten
        if(el.dataset._galleryWired !== "1"){
          el.dataset._galleryWired = "1";

          // Untuk beberapa device, tap cepat pada tombol bisa double-fire.
          // Jadi kita buat guard kecil.
          let last = 0;
          const guardClick = (e)=>{
            const t = Date.now();
            if(t - last < 350){
          ...
          el.addEventListener("click", guardClick, { passive:false });
          el.addEventListener("touchend", guardClick, { passive:false });

          // Jika user memilih file yang sama, change event kadang tidak terpanggil,
          // jadi reset value ketika focus agar bisa pilih ulang file yang sama.
          el.addEventListener("focus", ()=>{
            // jangan selalu kosongkan kalau Anda tidak mau,
            // tapi ini biasanya membantu di mobile.
            // el.value = "";
          }, { passive:true });
        }
      };
